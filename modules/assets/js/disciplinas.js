...
const renderTableSubjects = (data) => {
  ...
  const desktopRows = data
    .map((item) => {
      ...
      let actionsHtml = "";
      if (canHistory) actionsHtml += `<button class="btn-icon-action text-warning" style="width: 32px; height: 32px; padding: 0;" onclick="openAudit('education.subjects', ${item.subject_id}, this)" title="Auditoria/Log"><i class="fas fa-history" style="font-size: 0.85rem;"></i></button>`;
