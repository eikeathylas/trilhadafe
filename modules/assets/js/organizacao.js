...
window.getDiocese = async () => {
  ...
  } catch (e) { window.alertErrorWithSupport("Listar Dioceses", e.message); }
};
...
const renderTableDiocese = (data) => {
  ...
  const desktopRows = data.map((item) => {
    ...
    return `
      <tr>
          <td class="text-center align-middle ps-3" style="width: 65px;">${_renderLogoCircle(item.logo_url, item.display_name, item.org_id)}</td>
          <td class="align-middle">
              <div class="fw-bold text-body" style="font-size: 0.95rem;">${item.display_name}</div>
              <div class="text-muted small mt-1">${item.phone_main || "Sem telefone"}</div>
          </td>
          ...
          <td class="text-center align-middle" style="width: 130px;">
              <div class="d-flex align-items-center justify-content-center gap-2">
                  <div class="form-check form-switch m-0 p-0 d-flex align-items-center"><input class="form-check-input shadow-none m-0" type="checkbox" ${isActive ? "checked" : ""} onchange="toggleDio(${item.org_id}, this)" style="width: 44px; height: 24px; cursor: pointer;"></div>
    ...
    return `
      <div class="ios-list-item">
          ...
          <div class="flex-grow-1 d-flex flex-column justify-content-center py-1" style="min-width: 0;">
              <h6 class="fw-bold text-body m-0 text-truncate">${item.display_name}</h6>
              <div class="small text-secondary mt-1 d-flex align-items-center text-truncate"><i class="fas fa-map-marker-alt me-1 opacity-50"></i> ${item.city_state || "Local não informado"}</div>
          </div>
          <div class="d-flex flex-column align-items-end justify-content-center ms-2 gap-3" style="min-width: 90px;">
              <div class="d-flex align-items-center justify-content-end gap-2 w-100">
                  <div class="form-check form-switch m-0 p-0 d-flex align-items-center position-relative"><input class="form-check-input m-0 shadow-none" type="checkbox" ${isActive ? "checked" : ""} onchange="toggleDio(${item.org_id}, this)" style="width: 44px; height: 24px;"></div>
                  <div class="status-text-org-${item.org_id} d-flex align-items-center">${statusIconHtml}</div>
              </div>
              <div class="d-flex gap-2">
                  <button class="ios-action-pill text-warning bg-warning bg-opacity-10" onclick="openAudit('organization.organizations', ${item.org_id}, this)"><i class="fas fa-bolt"></i></button>
                  <button class="ios-action-pill text-primary bg-primary bg-opacity-10" onclick="modalInstituicao(${item.org_id}, this)"><i class="fas fa-pen"></i></button>
                  <button class="ios-action-pill text-danger bg-danger bg-opacity-10" onclick="deleteOrg(${item.org_id})"><i class="fas fa-trash"></i></button>
              </div>
          </div>
      </div>`;
  }).join("");
  ...
  container.html(`
    <div class="d-none d-md-block table-responsive"><table class="table-custom"><thead><tr><th colspan="2" class="ps-3 text-uppercase small opacity-75">Diocese / Instituição</th><th class="text-center text-uppercase small opacity-75">Tipo</th><th class="text-uppercase small opacity-75">Localização</th><th class="text-center text-uppercase small opacity-75">Estado</th><th class="text-end pe-4 text-uppercase small opacity-75">Ações</th></tr></thead><tbody>${desktopRows}</tbody></table></div>
...
window.getOrganizacoes = async () => {
  ...
  } catch (e) { window.alertErrorWithSupport("Listar Paróquias", e.message); }
};
...
const renderTableOrgs = (data) => {
  ...
  const desktopRows = data.map((item) => {
    ...
    return `
      <tr>
          ...
          <td class="align-middle">
              <div class="fw-bold text-body" style="font-size: 0.95rem;">${item.display_name}</div>
              <small class="text-secondary fw-medium">${item.phone_main || "-"}</small>
          </td>
          ...
          <td class="text-center align-middle" style="width: 130px;">
              <div class="d-flex align-items-center justify-content-center gap-2">
                  <div class="form-check form-switch m-0 p-0 d-flex align-items-center position-relative"><input class="form-check-input m-0 shadow-none" type="checkbox" ${isActive ? "checked" : ""} onchange="toggleOrg(${item.org_id}, this)" style="width: 44px; height: 24px; cursor: pointer;"></div>
                  <div class="status-text-org-${item.org_id} d-flex align-items-center">${statusIconHtml}</div>
              </div>
          </td>
          <td class="text-end align-middle pe-3">
  ...
  const mobileRows = data.map((item) => {
    ...
    return `
      <div class="ios-list-item">
          ...
          <div class="flex-grow-1 d-flex flex-column justify-content-center py-1" style="min-width: 0;">
              <h6 class="fw-bold text-body m-0 text-truncate">${item.display_name}</h6>
              <div class="small text-secondary mt-1 d-flex align-items-center text-truncate"><i class="fas fa-map-marker-alt me-1 opacity-50"></i> ${item.city_state || "Sem local"}</div>
          </div>
          <div class="d-flex flex-column align-items-end justify-content-center ms-2 gap-3" style="min-width: 90px;">
              <div class="d-flex align-items-center justify-content-end gap-2 w-100">
                  <div class="form-check form-switch m-0 p-0 d-flex align-items-center position-relative"><input class="form-check-input m-0 shadow-none" type="checkbox" ${isActive ? "checked" : ""} onchange="toggleOrg(${item.org_id}, this)" style="width: 44px; height: 24px;"></div>
                  <div class="status-text-org-${item.org_id} d-flex align-items-center">${statusIconHtml}</div>
              </div>
              <div class="d-flex gap-2">
                  <button class="ios-action-pill text-warning bg-warning bg-opacity-10" onclick="openAudit('organization.organizations', ${item.org_id}, this)"><i class="fas fa-bolt"></i></button>
  ...
  container.html(`
    <div class="d-none d-md-block table-responsive" style="overflow-x: visible;"><table class="table-custom"><thead><tr><th colspan="2" class="ps-3 text-uppercase small opacity-75">Paróquia / Comunidade</th><th class="text-center text-uppercase small opacity-75">Tipo</th><th class="text-uppercase small opacity-75">Localização</th><th class="text-center text-uppercase small opacity-75">Estado</th><th class="text-end pe-4 text-uppercase small opacity-75">Ações</th></tr></thead><tbody>${desktopRows}</tbody></table></div>
    <div class="d-md-none ios-list-container">${mobileRows}</div>`);
...
window.getLocais = async () => {
  ...
  } catch (e) { window.alertErrorWithSupport("Listar Locais", e.message); }
};
    ...
    return `
      <tr>
          <td class="text-center align-middle ps-3" style="width: 60px;">
              <div class="icon-circle bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-10 shadow-sm"><i class="fas fa-door-open"></i></div>
          </td>
          <td class="align-middle">
          ...
          <td class="text-center align-middle" style="width: 130px;">
              <div class="d-flex align-items-center justify-content-center gap-2">
                  <div class="form-check form-switch m-0 p-0 d-flex align-items-center position-relative"><input class="form-check-input m-0 shadow-none" type="checkbox" ${isActive ? "checked" : ""} onchange="toggleLoc(${item.location_id}, this)" style="width: 44px; height: 24px; cursor: pointer;"></div>
      ...
      <div class="ios-list-item flex-column align-items-stretch">
          <div class="d-flex justify-content-between align-items-start">
              <div class="flex-grow-1 pe-3">
                  <h6 class="fw-bold text-body m-0 text-truncate" style="font-size: 1.05rem;">${item.name}</h6>
                  <div class="small text-secondary mt-1 d-flex align-items-center"><i class="fas fa-users me-1 opacity-50"></i> Cap: ${item.capacity || 0} pessoas</div>
              </div>
              <div class="d-flex align-items-center gap-2 flex-shrink-0">
                  <div class="form-check form-switch m-0 p-0 d-flex align-items-center position-relative"><input class="form-check-input m-0 shadow-none" type="checkbox" ${isActive ? "checked" : ""} onchange="toggleLoc(${item.location_id}, this)" style="width: 44px; height: 24px;"></div>
                  <div class="status-text-loc-${item.location_id} d-flex align-items-center">${statusIconHtml}</div>
              </div>
          </div>
          ...
          <div class="d-flex justify-content-end gap-2 pt-3 mt-3 border-top border-secondary border-opacity-10">
              <button class="ios-action-pill text-warning bg-warning bg-opacity-10" onclick="openAudit('organization.locations', ${item.location_id}, this)"><i class="fas fa-bolt"></i></button>
              <button class="ios-action-pill text-primary bg-primary bg-opacity-10" onclick='editarLocalObj(JSON.parse(decodeURIComponent("${itemStr}")), this)'><i class="fas fa-pen"></i></button>
              <button class="ios-action-pill text-danger bg-danger bg-opacity-10" onclick="deleteLoc(${item.location_id})"><i class="fas fa-trash"></i></button>
          </div>
      </div>`;
  }).join("");

  container.html(`
    <div class="d-none d-md-block table-responsive" style="overflow-x: visible;"><table class="table-custom"><thead><tr><th colspan="2" class="ps-3">Espaço / Sala</th><th class="text-center">Lotação</th><th class="text-center">Recursos</th><th class="text-center">Ativo</th><th class="text-end pe-4">Ações</th></tr></thead><tbody>${desktopRows}</tbody></table></div>
    <div class="d-md-none ios-list-container">${mobileRows}</div>`);
...
$("#image-upload-container-org").on("click", function (e) {
  if (!$(e.target).closest("#btn-remove-logo").length) $("#org_photo").click();
});
...
$("#org_photo").change(function () {
  if (this.files && this.files[0]) {
    ...
    reader.onload = (e) => {
      ...
      $("#placeholder-logo-org").addClass("d-none");
...
window.removeLogoOrg = () => {
  ...
  $("#placeholder-logo-org").removeClass("d-none");
...
window.salvarInstituicao = async (btn) => {
  ...
  const data = {
    org_id: orgId, display_name: nome, legal_name: $("#org_legal_name").val(),
    org_type: $("#org_type").val(), tax_id: $("#org_tax_id").val(), phone_main: $("#org_phone").val(),
    email_contact: $("#org_email").val(), website_url: $("#org_website").val(), zip_code: $("#org_zip").val(),
    address_street: $("#org_street").val(), address_number: $("#org_number").val(), address_district: $("#org_district").val(),
    address_city: $("#org_city").val(), address_state: $("#org_state").val(), patron_saint: $("#org_patron").val()
  };
  ...
  try {
    ...
    if (result.status) {
      ...
      getDiocese(); getOrganizacoes();
    } else throw new Error(result.alert || "Erro ao salvar.");
  } catch (e) { window.alertErrorWithSupport(orgId ? `Editar Org` : "Criar Org", e.message); }
  finally { window.setButton(false, btn); }
};
...
window.deleteOrg = (id) => {
  Swal.fire({ title: "Excluir Instituição?", text: "Ela será movida para a lixeira do sistema.", icon: "warning", showCancelButton: true, confirmButtonColor: "#d33", confirmButtonText: "Sim, excluir" }).then(async (r) => {
    if (r.isConfirmed) {
      ...
      if (res.status) { window.alertDefault("Removida com sucesso.", "success"); getDiocese(); getOrganizacoes(); }
...
window.salvarLocal = async (btn) => {
  ...
  btn = $(btn); window.setButton(true, btn, " Salvando...");

  try {
    const data = {
      location_id: $("#loc_id").val(), org_id: orgId, name: nome, capacity: $("#loc_capacity").val(),
      responsible_id: $("#loc_responsible").val(), has_ac: $("#loc_ac").is(":checked"),
      is_accessible: $("#loc_access").is(":checked"), is_consecrated: $("#loc_sacred").is(":checked"),
      resources: JSON.stringify({
        whiteboard: $("#loc_whiteboard").is(":checked"), projector: $("#loc_projector").is(":checked"),
        sound: $("#loc_sound").is(":checked"), wifi: $("#loc_wifi").is(":checked"),
        kitchen: $("#loc_kitchen").is(":checked"), parking: $("#loc_parking").is(":checked"),
        fan: $("#loc_fan").is(":checked"), water: $("#loc_water").is(":checked"), computer: $("#loc_computer").is(":checked")
      }),
      address_zip: $("#loc_zip").val(), address_street: $("#loc_street").val(),
      address_number: $("#loc_number").val(), address_district: $("#loc_district").val()
    };
    const res = await window.ajaxValidator({ validator: "saveLocation", token: window.defaultApp.userInfo.token, data: data });
    if (res.status) { window.alertDefault("Local salvo!", "success"); $("#modalLocal").modal("hide"); getLocais(); }
    else throw new Error(res.alert);
  } catch (e) { window.alertErrorWithSupport("Salvar Local", e.message); }
  finally { window.setButton(false, btn); }
};
...
window.deleteLoc = (id) => {
  Swal.fire({ title: "Excluir Local?", text: "Será movido para a lixeira.", icon: "warning", showCancelButton: true, confirmButtonColor: "#d33", confirmButtonText: "Sim, excluir" }).then(async (r) => {
    if (r.isConfirmed) {
      ...
      if (res.status) { window.alertDefault("Local removido.", "success"); getLocais(); }
...
window.modalInstituicao = (id = null, btn = false) => {
  ...
  removeLogoOrg();
  if (btn) btn = $(btn);
  if ($("#org_type")[0]?.selectize) { $("#org_type")[0].selectize.clear(true); $("#org_type")[0].selectize.enable(); }
...
window.modalDiocese = () => {
  ...
  if ($("#org_type")[0]?.selectize) { $("#org_type")[0].selectize.setValue("DIOCESE"); $("#org_type")[0].selectize.disable(); }
};
...
const loadOrgData = async (id, btn) => {
  try {
    ...
    if (result.status) {
      const d = result.data;
      $("#org_id").val(d.org_id); $("#org_display_name").val(d.display_name);
      $("#org_legal_name").val(d.legal_name); $("#org_tax_id").val(d.tax_id);
      $("#org_phone").val(d.phone_main); $("#org_email").val(d.email_contact);
      $("#org_zip").val(d.zip_code); $("#org_street").val(d.address_street);
      $("#org_number").val(d.address_number); $("#org_district").val(d.address_district);
      $("#org_city").val(d.address_city); $("#org_state").val(d.address_state);
  ...
  } catch (e) { window.alertErrorWithSupport("Carregar Org", e.message); }
  finally { window.setButton(false, btn); }
};
...
const editarLocalObj = (item, btn) => {
  loadResponsibles().then(() => {
    ...
    $("#loc_id").val(item.location_id); $("#loc_name").val(item.name); $("#loc_capacity").val(item.capacity);
    ...
    $("#loc_zip").val(item.address_zip); $("#loc_street").val(item.address_street);
    $("#loc_number").val(item.address_number); $("#loc_district").val(item.address_district);
    modal.modal("show");
  });
};
...
const loadResponsibles = async () => {
  ...
  } catch (e) { console.error("Erro ao carregar responsáveis."); }
};

window.paginateWrapper = (page, funcName, type) => {
  if (type === "org") defaultOrg.orgCurrentPage = page;
  else if (type === "loc") defaultOrg.locCurrentPage = page;
  window[funcName]();
};

const _generatePaginationButtons = (containerClass, currentPageKey, totalPagesKey, funcName, type) => {
  let container = $(`.${containerClass}`); container.empty();
  let total = defaultOrg[totalPagesKey]; let current = defaultOrg[currentPageKey];
  if (total <= 1) return;

  let html = `<button onclick="paginateWrapper(1, '${funcName}', '${type}')" class="btn btn-sm btn-secondary me-1 shadow-sm" ${current === 1 ? "disabled" : ""}>Primeira</button>`;
  for (let p = Math.max(1, current - 2); p <= Math.min(total, current + 2); p++) {
    html += `<button onclick="paginateWrapper(${p}, '${funcName}', '${type}')" class="btn btn-sm ${p === current ? "btn-primary" : "btn-secondary"} me-1 shadow-sm">${p}</button>`;
  }
  html += `<button onclick="paginateWrapper(${total}, '${funcName}', '${type}')" class="btn btn-sm btn-secondary shadow-sm" ${current === total ? "disabled" : ""}>Última</button>`;
  container.html(html);
};
...
const populateOrgSelects = (data) => {
  ...
  $(".select-orgs, .select-orgs-modal").each((_, el) => {
    ...
    $el.selectize({
      valueField: "id", labelField: "title", searchField: ["title"], options: options, placeholder: "Selecione...",
...
$(document).ready(() => {
  ...
  $("#filtro-org-locais").change(() => { defaultOrg.locCurrentPage = 1; getLocais(); });
});